...
class Captcha {
	...
	function validate($request,$captchaVar) {
		...
		$key = $this->session->get('tmLibCaptchKey');
		$this->session->remove('tmLibCaptchKey');
	...
	private function generateImage() {
		...
		imageline($this->captcha,rand(30,50),0,rand(2,70),rand(15,30),$lime);
		imageline($this->captcha,rand(60,90),0,rand(50,85),rand(22,44),$lime);
		$this->centeredImageString($this->captcha, 5, $this->generateKey(), $textCol);
	}
	...
	private function createBlobs($img,$color,$number=15) {
		$imgW = imagesx($img);
		$imgH = imagesy($img);
		for($i=0;$i<$number;$i++)
		{
			$xstart = rand(0,$imgW);
			$xstop = rand($xstart,$xstart + 2);
			$ystart = rand(0,$imgH);
			$ystop = rand($ystart,$ystart + 2);
	...
	private function generateKey() {
		...
		$this->session->set('tmLibCaptchKey',md5($key));
		return $key;
	}
}
?>